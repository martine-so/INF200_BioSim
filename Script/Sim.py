...
class BioSim:
    ...
    def simulate(self, num_years):
        ...
        for i in self.ini_pop:
            animals = [Herbivore(j['age'], j['weight'], self.seed) for j in i['pop']]

        num_animals = []
        num_years_list = list(range(1, num_years+1))
        for year in range(num_years):
            field = land_type(animals)
            field.eating()
            field.breeding()
            # print(len(animals))
            field.aging()
        ...
        plt.figure()
        plt.plot(num_years_list, num_animals)
